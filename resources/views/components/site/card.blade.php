    ...
    <div class="border-0 shadow-md card h-100 rounded-0">
        <div class="row h-100">
            <div class="col-md-5" style="overflow: hidden">
                <img onerror="this.src=`http://placehold.jp/99ccff/003366/{{ $imageSize }}.png?text={{ $props->title }}`"
                    data-src="{{ route('resizes', ['size' => '200x200', 'imagePath' => $props->thumb]) }}"
                    alt="{{ $props->title }}"
...
@else
    <div class="border-0 shadow-md card h-100 rounded-0">
        @if ($hasImage === true)
            <img class=" rounded-0 card-img-top" data-src="{{ resizeImage($props->thumb, $imageSize) }}"
                onerror="this.src=`http://placehold.jp/99ccff/003366/{{ $imageSize }}.png?text={{ $props->title }}`"
